...
function ensureOfflineGateStyles() {
  ...
  style.textContent = `
    ...
    .og-title{
      margin:14px 0 6px; font-size:28px; line-height:1.08;
      font-weight:1000; color:#fff; letter-spacing:-.6px;
    }
    .og-sub{
      margin:0; font-size:14px; line-height:1.55;
      font-weight:800; color:rgba(255,255,255,.90);
    }
    .og-body{ padding:18px; display:grid; gap:12px; }
    .og-info{
      border-radius:20px; border:1px solid rgba(255,255,255,.08);
      background:rgba(255,255,255,.04); padding:14px;
    }
    .og-label{
      font-size:11px; font-weight:1000; color:rgba(255,255,255,.50);
      text-transform:uppercase; letter-spacing:.8px; margin-bottom:5px;
    }
    .og-value{
      font-size:20px; font-weight:1000; color:#fff; line-height:1.15;
    }
    .og-desc{
      margin:2px 0 0; font-size:12px; line-height:1.45;
      font-weight:800; color:rgba(255,255,255,.62);
    }
    .og-actions{ display:grid; gap:10px; margin-top:2px; }
    .og-btn{
      min-height:54px; border:none; border-radius:18px; cursor:pointer;
      font-size:15px; font-weight:1000; transition:transform .16s ease;
    }
    .og-btn:active{ transform:scale(.985); }
    .og-btn.primary{
      color:#fff;
      background:linear-gradient(135deg,#a5b4fc 0%,#6366f1 50%,#ec4899 100%);
  ...
  wrap.innerHTML = `
    <div class="og-card">
      <div class="og-top">
        ...
        <p class="og-sub" id="ogSub">Seçilen dil ile İngilizce arasındaki çift yönlü iki dosya 12 ay açılır.</p>
      </div>

      <div class="og-body">
        <div class="og-info">
          <div class="og-label">Paket içeriği</div>
          <div class="og-value">Dil ↔ İngilizce</div>
          <p class="og-desc">Örnek: DE→EN ve EN→DE birlikte açılır.</p>
        </div>

        <div class="og-info">
          <div class="og-label">Ücret ve süre</div>
          <div class="og-value">5 Jeton • 12 Ay</div>
          <p class="og-desc">TR↔EN ücretsizdir. Diğer diller oyun değil, uzun ömürlü paket gibi çalışır.</p>
        </div>
...
export function openOfflineGateModal(options = {}) {
  ...
  const {
    ...
    message = "Seçilen dil ile İngilizce arasındaki çift yönlü iki dosya 12 ay açılır."
  } = options;
...
export async function ensureOfflineLangAccess(lang = "") {
  ...
  if (!code) {
    return { ok: false, access_open: false, reason: "LANG_REQUIRED" };
  }
  ...
  try {
    ...
    if (!json.ok && json.reason === "INSUFFICIENT_TOKENS") {
      openOfflineGateModal({
        title: "Bu offline dil paketi için 5 jeton lazım",
        message: `Bu dil için ${PACK_PRICE} jeton gerekir. Paket açılınca ${PACK_YEARS_TEXT} boyunca ${code.toUpperCase()}↔EN çalışır.`
      });
      return {
        ok: false,
        access_open: false,
