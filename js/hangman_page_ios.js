...
function getBackUrl() {
  return qs.get("back") || "/pages/game_menu_ios.html?ios=1";
}

function speakGameText(text, lang) {
  const clean = String(text || "").trim();
  if (!clean || !("speechSynthesis" in window)) return false;
  try {
    const bcp = GAME_LANG_META[lang]?.tts || GAME_LANG_META[lang]?.bcp || GAME_LANG_META[lang]?.locale || "en-US";
