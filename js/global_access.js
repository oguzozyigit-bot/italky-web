...
function goLogin() { try { const here = encodeURIComponent(location.pathname + location.search + location.hash); location.replace(`/pages/login.html?next=${here}`); } catch { location.href = "/pages/login.html"; } }
function goMembership() { try { const here = encodeURIComponent(location.pathname + location.search + location.hash); location.replace(`${MEMBERSHIP_URL}?expired=1&next=${here}`); } catch { location.href = MEMBERSHIP_URL; } }
function goCodeLoad() { try { const here = encodeURIComponent(location.pathname + location.search + location.hash); location.replace(`${CODE_LOAD_URL}?next=${here}`); } catch { location.href = CODE_LOAD_URL; } }
...
function hydrateMenuAccessBadge(access) {
  try {
    ...
    const badge = document.getElementById("menuAccessTime"); const label = access?.remaining_label || formatRemaining(access?.remaining_seconds || 0);
    if (badge) { badge.textContent = access?.access_open ? label : "Kullanım süresi doldu"; badge.style.color = access?.access_open ? "#8bd3ff" : "#ffb4b4"; }
  } catch {}
}

function setCachedAccess(access) { try { window.__ITALKY_ACCESS__ = access; localStorage.setItem("italky_access_state", JSON.stringify(access)); hydrateMenuAccessBadge(access); } catch {} }
