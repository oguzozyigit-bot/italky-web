...
const ALWAYS_OPEN_PATHS = new Set([
  "/pages/home.html", "/pages/membership.html", "/pages/code_load.html", "/pages/about.html", "/pages/privacy.html", "/pages/contact.html", "/pages/profile.html", "/pages/login.html", "/pages/auth_callback.html"
]);
...
function isHomePath(pathname = location.pathname) { const p = normalizePath(pathname); return p === "/pages/home.html" || p === "/home.html"; }
function isOfflineLanguagePath(pathname = "") { return normalizePath(pathname) === "/pages/offline_languages.html"; }

function getActiveUntil(access = {}) {
  const candidates = [access?.active_until, access?.membership_ends_at, access?.package_ends_at, access?.trial_ends_at, access?.subscription_ends_at, access?.gift_ends_at].map(parseTime).filter(Boolean);
...
function standardizeShellSignature() {
  try {
    ...
    installMenuFocusStyles();
  } catch {}
}

function installMenuFocusStyles() {
  try {
    if (document.getElementById("italkyMenuFocusStyle")) return;
    const st = document.createElement("style");
    st.id = "italkyMenuFocusStyle";
    st.textContent = `
      ...
      .menu-action[data-focus-action],.menu-nav a[data-focus-action],#widgetShortcutBtn,#addWidgetBtn,#shortcutBtn,#addShortcutBtn{position:relative;overflow:hidden;animation:italkySoftPulse 2.8s ease-in-out infinite;border-color:rgba(139,211,255,.24)!important;box-shadow:0 0 0 1px rgba(139,211,255,.06),0 0 20px rgba(139,211,255,.10)!important;}
      .menu-action[data-focus-action]::after,.menu-nav a[data-focus-action]::after,#widgetShortcutBtn::after,#addWidgetBtn::after,#shortcutBtn::after,#addShortcutBtn::after{content:"";position:absolute;inset:0;background:linear-gradient(110deg,transparent,rgba(255,255,255,.12),transparent);transform:translateX(-120%);animation:italkySoftSweep 3.6s ease-in-out infinite;pointer-events:none;}
      @keyframes italkySoftPulse{0%,100%{filter:brightness(1);box-shadow:0 0 0 1px rgba(139,211,255,.06),0 0 14px rgba(139,211,255,.08)}50%{filter:brightness(1.08);box-shadow:0 0 0 1px rgba(139,211,255,.16),0 0 24px rgba(139,211,255,.16)}}
      @keyframes italkySoftSweep{0%,45%{transform:translateX(-120%)}72%,100%{transform:translateX(120%)}}
    `;
    document.head.appendChild(st);
    requestAnimationFrame(markWidgetShortcutActions);
  } catch {}
}

function markWidgetShortcutActions() {
  try {
    const words = ["widget", "kısayol", "kisayol", "shortcut"];
    document.querySelectorAll(".menu-nav a,.menu-action,button,a").forEach((el) => {
      const txt = cleanLower(el.textContent || el.getAttribute("aria-label") || el.id || "");
      if (words.some((w) => txt.includes(w))) el.setAttribute("data-focus-action", "1");
    });
  } catch {}
}
...
function goMembership() { resetMobileViewportState(); try { const here = encodeURIComponent(location.pathname + location.search + location.hash); location.href = `${MEMBERSHIP_URL}?expired=1&next=${here}`; } catch { location.href = MEMBERSHIP_URL; } }
function goCodeLoad() { resetMobileViewportState(); try { const here = encodeURIComponent(location.pathname + location.search + location.hash); location.href = `${CODE_LOAD_URL}?next=${here}`; } catch { location.href = CODE_LOAD_URL; } }
...
function hydrateMenuAccessBadge(access) {
  try {
    ...
    standardizeShellSignature(); markWidgetShortcutActions();
  } catch { standardizeShellSignature(); }
}

function closeGateModal() { try { document.getElementById("italkyAccessGateModal")?.remove(); document.getElementById("italkyAccessExpiredModal")?.remove(); } catch {} }
...
function showGateModal({ title, text, buttons = [] }) {
  try {
    ...
    if (!document.getElementById("italkyAccessGateStyle")) {
      const st = document.createElement("style"); st.id = "italkyAccessGateStyle"; st.textContent = `.italkyGateBtn{width:100%;min-height:46px;border:0;border-radius:16px;color:#fff;font-size:14px;font-weight:1000;font-family:Outfit,system-ui,sans-serif}.italkyGateBtn.primary{background:linear-gradient(135deg,#ff5a5f,#ff2d55)}.italkyGateBtn.secondary{background:linear-gradient(135deg,#f59e0b,#a855f7)}.italkyGateBtn.secondary[data-action="continue"]{background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.10)}`; document.head.appendChild(st);
    }
  } catch {}
}
...
function showOfflineDownloadBlockedPrompt() {
  showGateModal({ title:"Offline indirme için süre yetersiz", text:"Offline dil paketi indirmek için kullanım sürenizin en az 24 saat olması gerekir. Lütfen gün yükleyerek devam edin.", buttons:[{ label:"Gün Yükle", action:"membership" }, { label:"Kod ile Gün Yükle", action:"code" }] });
}
...
export async function initGlobalAccess(options = {}) {
  ...
  if (codeState) { const codeAccess = buildCodeAccess(codeState); setCachedAccess(codeAccess); dispatchAccessReady(codeAccess); if (!codeAccess.access_open && currentPath !== "/pages/membership.html" && currentPath !== "/pages/code_load.html" && !isHomePath(currentPath)) await showAccessExpiredPrompt(); return { ok:true, code_access:true, session:null, access:codeAccess }; }
