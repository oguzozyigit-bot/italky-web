...
function ensureGameGateStyles() {
  ...
  style.textContent = `
    ...
    .gg-body{ padding:18px; display:grid; gap:12px; }
    .gg-info{
      border-radius:20px; border:1px solid rgba(255,255,255,.08);
      background:rgba(255,255,255,.04); padding:14px 14px;
    }
    .gg-label{
    ...
    .gg-actions{ display:grid; gap:10px; margin-top:2px; }
    .gg-btn{
      min-height:54px; border:none; border-radius:18px; cursor:pointer;
      font-size:15px; font-weight:1000; transition:transform .16s ease, opacity .16s ease;
    }
    .gg-btn:active{ transform:scale(.985); }
    .gg-btn.primary{
      color:#fff;
      background:linear-gradient(135deg,#a5b4fc 0%,#6366f1 50%,#ec4899 100%);
...
export async function ensureGamesBundleAccess(gameCode = "") {
  ...
  try {
    ...
    if (!json.ok && json.reason === "INSUFFICIENT_TOKENS") {
      openGamesGateModal({
        title: "Bu oyun için 1 jeton lazım",
        message: `Bu oyuna giriş için ${GAME_PRICE} jeton gerekir. Açılınca ${GAME_DAYS} gün boyunca tekrar jeton istemez.`
      });
      return {
        ok: false,
        access_open: false,
