...
const BALKAN_POOL = [
  ...
  { code: "sr", name: "Sırpça", flag: "🛡️" },
  { code: "hr", name: "Hırvatça", flag: "🌊" },
  { code: "mk", name: "Makedonca", flag: "☀️" },
  { code: "bg", name: "Bulgarca", flag: "🌹" },
  { code: "ro", name: "Romence", flag: "🏛️" },
...
async function runTranslateText(fromSide, text) {
  ...
  try {
    ...
    if (fromSide === "top") {
      translated = await translateAI(cleanText, currentBalkan().code, "tr");
      speakLang = "tr";
    } else {
      translated = await translateAI(cleanText, "tr", currentBalkan().code);
      speakLang = currentBalkan().code;
    }
...
function startRecognition(side) {
  ...
  const langCode = side === "top" ? currentBalkan().code : "tr";
...
function bindEvents() {
  ...
  UI.topSettingsMini?.addEventListener("click", () => {
    location.href = "/pages/premium_voice_settings.html?from=balkanlarin_dili";
  });
