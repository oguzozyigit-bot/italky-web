...
function showFatal(msg){
  try{
    ...
    box.style.color="#fff";
    box.style.fontFamily="Outfit,system-ui,sans-serif";
    box.style.textAlign="center";
    box.innerHTML = `
      <div style="font-weight:900; font-size:18px; margin-bottom:10px;">WalkieTalkie Açılmadı</div>
      <div style="opacity:.75; font-weight:700; font-size:13px; line-height:1.4;">${String(msg||"Bilinmeyen hata")}</div>
      <button id="wtBackBtn" style="margin-top:16px; height:46px; padding:0 16px; border-radius:14px; border:1px solid rgba(255,255,255,.2); background:rgba(255,255,255,.08); color:#fff; font-weight:900;">
        Geri Dön
      </button>
    `;
    document.body.appendChild(box);
    document.getElementById("wtBackBtn")?.addEventListener("click", ()=>location.href="/pages/f2f_connect.html");
  }catch{
    alert(msg);
...
function requireEl(id){
  const el = $(id);
  if(!el) throw new Error(`Eksik element: #${id} (f2f_call.html ID uyuşmuyor)`);
  return el;
}

window.addEventListener("error",(e)=>{
  console.warn("[WT ERROR]", e?.message || e);
});
...
function boot(){
  ...
  const micBtn = requireEl("micBtn");
  const langSelect = requireEl("langSelect");
  const peopleScroll = requireEl("peopleScroll");
  const peopleCount = requireEl("peopleCount");
  ...
  async function requestFloor(){
    ...
    try{ ws.send(JSON.stringify({ type:"floor_request" })); return true; }catch{ return false; }
  }
  ...
  function bindPushToTalk(btn){
    const down = async (e)=>{
      ...
      if(!ok){ pressActive = false; return; }

      setTimeout(async ()=>{
        if(!pressActive) return;
        if(myHasFloor) await startRecord();
        else showBanner("🎙️ Sinyal alınıyor…");
      }, 120);
    };
