...
const IOS_HOME_URL = "/pages/home_ios.html?ios=1";
const IOS_LOGIN_URL = "/pages/login_ios.html";
...
async function speakText(text, langCode) {
  ...
  if (!window.speechSynthesis) {
    toast("Bu cihazda ses oynatma desteklenmiyor veya izin gerektiriyor.");
    return;
  }
  try {
...
async function requireLogin() {
  ...
  if (!session?.user) {
    location.replace(IOS_LOGIN_URL);
    return false;
  }
...
function bindEvents() {
  ...
  soundToggle?.addEventListener("click", () => {
    ...
    else speakText(resultBubble?.textContent || "", toLang);
  ...
  homeLink?.addEventListener("click", (event) => { event.preventDefault(); location.href = IOS_HOME_URL; });
  homeBtn?.addEventListener("click", () => { location.href = IOS_HOME_URL; });
