(()=>{
if(window.__italkyCallLiveTranslateV2)return;window.__italkyCallLiveTranslateV2=true;
...
#call.itCallLive .subs{left:12px!important;right:12px!important;bottom:106px!important;z-index:9!important;display:flex!important;flex-direction:column!important;gap:7px!important;max-height:34vh!important;overflow:hidden!important;pointer-events:none!important}
...
function ensureUI(){
 ...
 if(!stage.querySelector('.itCallStatus')){const s=d.createElement('div');s.className='itCallStatus';s.textContent='Hoparlör açık · canlı çeviri hazır';stage.appendChild(s)}
 ...
 if(!controls.querySelector('.itSpeakerCtl')){const b=d.createElement('button');b.type='button';b.className='ctl itSpeakerCtl itActive';b.textContent='🔊';b.onclick=async()=>{const remote=$('remote'),on=!b.classList.contains('itActive');b.classList.toggle('itActive',on);b.textContent=on?'🔊':'🔇';if(remote){remote.muted=!on;if(on){remote.volume=1;try{await remote.play()}catch{}}}};controls.insertBefore(b,controls.firstChild)}
 if(!controls.querySelector('.itCaptionCtl')){const b=d.createElement('button');b.type='button';b.className='ctl itCaptionCtl itActive';b.textContent='CC';b.onclick=()=>{const subs=$('subs'),on=!b.classList.contains('itActive');b.classList.toggle('itActive',on);if(subs)subs.style.display=on?'flex':'none'};controls.insertBefore(b,controls.querySelector('.hang'))}
 const remote=$('remote');if(remote){remote.muted=false;remote.volume=1;remote.autoplay=true;remote.play?.().catch(()=>{})}
}
